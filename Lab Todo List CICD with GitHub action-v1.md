...
<img width="1908" height="973" alt="image" src="https://github.com/user-attachments/assets/62150473-f23a-4e88-a98c-518d49266621" />
...
1 health_check() ↔ test_health_endpoint_success
ตรวจ /api/health ว่าเชื่อม DB ได้หรือไม่
คาดหวัง: status 200, status="healthy"

2 create_todo() ↔ test_create_todo_with_full_data
ทดสอบ POST /api/todos สร้าง todo ใหม่
คาดหวัง: status 201, success=True, title ตรงกับที่ส่ง

3 delete_todo() ↔ test_delete_todo
ทดสอบ DELETE /api/todos/<id> เพื่อลบ todo
คาดหวัง: status 200, success=True, และ GET ซ้ำต้อง 404
...
## บันทึกรูปผลการทำงาน
<img width="1920" height="951" alt="image" src="https://github.com/user-attachments/assets/ddcacfb8-5bcb-4b20-a198-fc55d7b1f9da" />
FAIL Required test coverage of 90% not reached. Total coverage: 6.25%
=========================== short test summary info ============================
ERROR tests/test_app.py - AttributeError: module 'sqlalchemy' has no attribute '__all__'
ERROR tests/test_config.py - AttributeError: module 'sqlalchemy' has no attribute '__all__'
...
1. **Docker Architecture**:
   - เหตุใดจึงต้องแยก database และ application เป็นคนละ containers ?
    แยก DB กับ App คนละ container 👉 จัดการง่าย ไม่พังพร้อมกัน แก้หรืออัปเดตแยกได้
   - Multi-stage build มีประโยชน์อย่างไร?
    Multi-stage build 👉 ทำให้ image เล็กลง เร็วขึ้น เพราะเก็บแค่ส่วนที่จำเป็นตอนรัน

2. **Testing Strategy**:
   - การวัด code coverage มีความสำคัญอย่างไร?
    Code coverage 👉 ดูว่าทดสอบครบแค่ไหน ช่วยหาจุดที่ยังไม่ได้เทสต์

3. **Deployment**:
   - Health check endpoint มีความสำคัญอย่างไร?
    Health check 👉 ไว้เช็กว่าเซิร์ฟเวอร์ยังทำงานดีไหม ก่อนเปิดให้ใช้งาน
   - Render และ Railway มีความแตกต่างกันอย่่างไร?
    Render vs Railway 👉 Render ตั้งค่าซับซ้อนกว่าแต่ยืดหยุ่นกว่า / Railway ใช้ง่ายกว่าเหมาะกับเริ่มต้น
